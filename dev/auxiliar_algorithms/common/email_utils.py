...
fs_cache = decorators.fs_cache("common")
...
def university_domains():
    ...
    True
    """
    fh = open(os.path.join(os.path.dirname(__file__), "email_university_domains.csv"))
...
def public_domains():
    ...
    True
    """
    fh = open(os.path.join(os.path.dirname(__file__), "email_public_domains.csv"))
    return set(addr_domain.strip() for addr_domain in fh)
...
def commercial_domains():
    ...
    return set(dus[~is_public_bulk(es) & ~is_university_bulk(es) & (dus > 1)].index)
...
def is_university(addr):
    ...
    return (
        (chunks[-1] == "edu" and chunks[-2] not in ("england", "australia"))
        or chunks[-2] == "edu"
